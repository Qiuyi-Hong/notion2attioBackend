...
A silent repair never applies to an **override**. The pipeline repairs what Notion gave it, not what the reviewer typed: a value that changed the instant it was entered has no earlier state to compare it with, so its history would say nothing.
...
A derived value lives in exactly one place — on the candidate that owns it — and reaches the files when they are written. It is never copied onto a second candidate, so it can never go stale in one place and stay correct in another.

## Override

A change the reviewer makes to a value the pipeline proposed. The counterpart to a **silent repair**: a repair is what the pipeline changed without asking, an override is what a human changed having been asked.

An override is taken exactly as typed, and it **pins** the value it lands on — an overridden value stops following whatever it was derived from. A value is overridden only when it differs from what the pipeline proposed; re-typing the same value changes nothing.

The reviewer cannot override the values a candidate's identity is keyed on: a Company candidate's normalised domain and a Person candidate's work email. Those change only through a flag's own control.

## Hold

The reviewer's act of keeping a candidate out of the files. Distinct from a Stop, which is the pipeline's judgement; a hold is the reviewer's, needs no reason, and is available on every candidate.
