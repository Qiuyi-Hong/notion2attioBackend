...
```
{
  ...
  screening:  { model, effort, prompt, entries } | null,   // the screening log
...
`screening` is the repair log's counterpart for the one model call the pipeline makes, on the same rule: not needing the reviewer's attention is not the same as being hidden ([ADR-0002](./adr/0002-a-model-may-only-raise-a-flag.md)). It carries the three things that decide what the screener does — `model`, `effort` and the `prompt` version, so a change to any of them is attributable — and one `entries` row per source row, holding what came back as `kept` and `discarded`. A suspicion is discarded when its `quote` is not an exact substring of that row's own notes, which is what makes an invented suspicion structurally unable to surface: the model points at evidence, and the pipeline checks the pointer. It is `null` before `check` has run, and after it when there was no key — which is exactly when `N0` appears.
