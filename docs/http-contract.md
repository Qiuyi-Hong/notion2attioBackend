...
```
{
  ...
  candidates: { companies: [...], people: [...], deals: [...] },  // per #10
                                     // a Person also carries `notes` — see below
  batchFlags: [{ id, rule, level, kind, cleared, refused, stage }],  // stage on P1+P2 only
...
- `rule` names the rule that raised it, and is what the surface renders a fixed sentence for. No prose travels on the wire, which is also what keeps the screener's model unable to narrate ([ADR-0002](./adr/0002-a-model-may-only-raise-a-flag.md)). The screener's notices are `N1`, `N2` and `N1+N2` — one flag per candidate however many kinds it earned, named the way `P1+P2` is, because one flag is one _problem_ and both kinds point at the single thing the reviewer can act on: _this person may already exist in Attio_. The quote a kind was raised on is **never** on a flag — it is a check, not a display, and it moves between identical runs ([#30](https://github.com/Qiuyi-Hong/notion2attioBackend/issues/30)). It is in the screening log below.
...
Every candidate also carries **`held`**, **`heldByReviewer`** and **`overrides`**. `held` is candidate state, read off the candidate's flags and the reviewer's holds and never typed in: a candidate carrying an uncleared Stop is Held, so is one the reviewer held, and so is every candidate in a Company's account when the Company is. It is computed **server-side, in one place**, because the browser deriving the same rule would let the two disagree — and the cascade is the one that decides what reaches Attio. `overrides` names the fields the reviewer pinned: an edit differing from what the pipeline proposed stops following whatever it was derived from, which is what makes ADR-0004's third provenance mark visible instead of hidden.

`heldByReviewer` is the reviewer's own hold alone, as against the `held` that is read off it. It travels because `held` in a decision document is **not** sparse and replaces what came before: a browser that reloaded the ledger and could see only the derived `held` would have to choose between posting the cascade back as holds in its own right and posting nothing, which takes the reviewer's holds away on their next edit. Re-sending exactly the ids this names is what leaves the ledger where the reviewer left it. It is not a second rule for the browser to apply — the cascade stays the server's — it is the reviewer's own act, read back.
...
The **screening log** is not on this wire. It is an audit record rather than a reviewer surface — the model, the effort, the prompt version, and every suspicion returned for every row including the ones the quote check discarded — and it lives in the run's checkpoint beside the repair log, which is the pipeline's own durable record ([ADR-0002](./adr/0002-a-model-may-only-raise-a-flag.md), [ADR-0009](./adr/0009-the-server-keeps-its-own-record-of-runs.md)). A discarded quote is the one piece of model-authored text a run holds, so keeping it off the wire is what makes *the quote span is never rendered* ([#60](https://github.com/Qiuyi-Hong/notion2attioBackend/issues/60)) structural rather than a promise the browser keeps.

**`Research notes` are on this wire, settled by [#60](https://github.com/Qiuyi-Hong/notion2attioBackend/issues/60)** — _"the full research notes show on every candidate, notice or not"_, which is also [ADR-0002](./adr/0002-a-model-may-only-raise-a-flag.md)'s *the reviewer always sees the full notes*. They sit on the **Person candidate**, as `notes: [{ sourceId, text }]`, because that is who they are written about — one contact at one company — and a value lives in exactly one place (ADR-0004). A Company and a Deal reach their account's notes through its People rather than holding a copy that could go stale, which is the same reach-through a Person's company name already travels by.

It is a list because two source rows sharing a work email collapse onto one Person candidate and each brought its own notes; each entry names the row it came from, exactly as a `Repair` does, so a collapse stays attributable. A row whose notes are empty produces no entry, on the repair log's rule — the ledger says so where the list is empty rather than rendering a blank quotation.

This does **not** put the model's quote span on the wire, and the two are not in tension only by assertion. A kept quote is by definition a verbatim substring of the notes, so once the notes travel the characters are unavoidable; what *the quote span is never rendered* means, and what `screening.test.mjs` checks, is that the span is nowhere on the wire **as a value of its own**. The Reviewer is handed their own text whole, with nothing marking where the model looked.
