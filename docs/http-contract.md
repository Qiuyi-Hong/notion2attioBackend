...
**`Research notes` are not on this wire either, and [#60](https://github.com/Qiuyi-Hong/notion2attioBackend/issues/60) needs them** — _"the full research notes show on every candidate, notice or not"_, which is also [ADR-0002](./adr/0002-a-model-may-only-raise-a-flag.md)'s *the reviewer always sees the full notes*. No candidate has carried them since [#52](https://github.com/Qiuyi-Hong/notion2attioBackend/issues/52), so the screener has removed nothing — but a notice currently reaches the reviewer with the evidence off screen. Where the notes sit is a candidate-shape question (one Person candidate can collapse two source rows, and so two notes), which is why it belongs to the ticket that puts the ledger on screen rather than to the screener.
